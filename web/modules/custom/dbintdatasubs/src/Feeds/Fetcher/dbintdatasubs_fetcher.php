...
namespace Drupal\dbintdatasubs\Feeds\Fetcher;
 ...
 *
 * @FeedsFetcher(
 *   id = "dbintdatasubs_fetcher",
 *   title = @Translation("Dashboard Interim Data Submissions Fetcher"),
 *   description = @Translation("Dashboard Interim Data Submissions Fetcher"),
 * )
 */
class DbIntDatasubs_Fetcher extends PluginBase implements FetcherInterface {

  /**
   * {@inheritdoc}
   */
  public function fetch(FeedInterface $feed, StateInterface $state) {
    $config = \Drupal::configFactory()->get('dbintdatasubs.settings');
    $result = $this->DbIntDatasubsFunction($config);
    if ($result !== FALSE) {
   ...
   */
  public function DbIntDatasubsFunction(ImmutableConfig $config) {
    //Get URL from settings.php
    $baseURL = Settings::get('api_url_base', 'https://api.vision-covid.org');
    $url = $baseURL . "/api/dashboard/interimdatasubmissionbysite";
